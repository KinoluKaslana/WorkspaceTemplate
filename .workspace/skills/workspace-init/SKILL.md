...
确认 `skills/REGISTRY.md` 有 `workspace-skills` 命名空间（template-update / workspace-init / git-init 三个入口）；`bootstrap.json` 的 `skills.registered_roots` 含 `workspace-skills`。缺则补登记（TOOLS.md 同步 §4.5 条目）。
